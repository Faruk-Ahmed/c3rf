/* This is a C version of Swersky et al.'s sum_cardinality.py
 * Faruk Ahmed, 2014
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for (i = 0; i < hammingRadius+1; i++)
        cardinalityFactor[i] = 0;
    for (i = hammingRadius+1; i < D+1; i++)
        cardinalityFactor[i] = MINUSINF;

    downMsgFromZ = mxCalloc(D+1, sizeof(double));
    for (i = 0; i < D+1; i++)
        downMsgFromZ[i] = cardinalityFactor[i] - log(hammingRadius+1);
    
    topFactorTerm = 0;
    
    if (calcMargs)
    {
        cardinalityFactorBelief = mxCalloc(hammingRadius+1, sizeof(double));
